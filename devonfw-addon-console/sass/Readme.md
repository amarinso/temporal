...
folder are mapped to the application's root namespace, 'DevonConsole'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "DevonConsole"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'DevonConsole' in the same way
as `"DevonConsole/sass/src"`.
